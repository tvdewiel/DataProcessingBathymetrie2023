...
namespace DataSetManager
{
    public class FileDataSetManager
    {
        ...
        public static (DataSet,List<GridDataSet>) MakeGridDataSetsWithTestSet(DataSet dataSet,int testSetSize,List<(int n,double delta)>size)
        {
            try
            {
                DataSet testDataSet;
                ...
                int n = 0;
                int index;
                if (testSetSize > dataSet.data.Count) throw new DataSetManagerException($"MakeDataSetsWithTestSet - size to high - 0,{size[0]}");
                while (n < testSetSize)
                {
                ...
                for (int i = 1; i < size.Count; i++)
                {
                    Dictionary<int, XYZ> set = new Dictionary<int, XYZ>();
                    n = 0;
                    if (size[i].n > dataSet.data.Count) throw new DataSetManagerException($"MakeDataSetsWithTestSet - size to high - {i},{size[i]}");
                    while (n < size[i].n)
                    {
                        index = random.Next(dataSet.data.Count);
                        if (!set.ContainsKey(index) && !testset.ContainsKey(index))
                    ...
                    sets.Add(new GridDataSet(dataSet.XYBoundary,size[i].delta, set.Values.ToList()));
                }
                return (testDataSet,sets);
            }
            catch(Exception ex) { throw new DataSetManagerException("makegriddataset"); }
        }
    }
}
